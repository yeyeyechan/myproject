...
def _recursive_dir_search_worker(dirlist, addr_target_dir, data_queue) :
    for entry in dirlist :
        try:
            #entry가 디렉토리인 경우
            if entry.is_dir(follow_symlinks = True):

                _recursive_dir_search_worker(addr_target_dir+separator+entry.name)
...
def  recursive_dir_search_master(addr_target_dir):
    global logf
    global data_queue
    ...
    dirlist = sorted(os.scandir(addr_target_dir),key=(lambda entry: entry.name)) # entry : os.DirEntry 객체
    l = int(len(dirlist)/2)
    dirlist1 = dirlist[0:l]
    dirlist2 = dirlist[l:]
    print('length : '+str(dirlist1))
    print('length : '+str(dirlist2))



    producer1 = multiprocessing.Process(target=_recursive_dir_search_worker,args=(dirlist1, addr_target_dir, data_queue))
    producer2 = multiprocessing.Process(target=_recursive_dir_search_worker,args=(dirlist2, addr_target_dir, data_queue))
...
if __name__ == "__main__" :
    ...
    recursive_dir_search_master(path)
