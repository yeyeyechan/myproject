...
'''
    ...
    **실행 결괴 :
        -모든 cpu core를 전부 사용는 상황을 보여준다
        -메모리 오버플로우 현상이 발생하지 않는다 (큐 maxsize 지정을 통한 해결)
        -문제점 : 
            -아무리 클라이언트에서 빠르게 처리하여 전송한다 하더라도 db에서 수신한 데이터를 저장하는 속도가 이를 따라가지 못하는 것으로 보인다 (GC Overhead limit exceeded 발생)
...
def split_data(path, num = NUM_OF_PRODUCERS):
    ...
    dir_list = sorted(scandir.scandir(path),key=(lambda entry: entry.name))
...
if __name__ == "__main__":
    ...
    print '(DEBUG)-TIME_ELAPSED : '+str(et-st)
